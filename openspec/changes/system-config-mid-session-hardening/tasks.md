...
- [x] 3.4 Delivered via `CanvasBlocksService.rebindModelSlot(modelName, newSlotIndex, newNodeId)` in `canvas-blocks.service.ts`. Reifies slot rebind: refreshes `slot_module` from current hardware via existing private `snapshotSlotModule` helper, then triggers one `updateAndPrint()` evaluate roundtrip. All entry points that reassign a model's slot must route through this to keep the snapshot in sync. Consumed by §10.3 Edit Model modal
...
- [x] 10.3 `edit-model-modal` standalone component scaffolded in `trigger-flow-ui/src/app/main-flow/edit-model-modal/`. Prefilled with name / slot / notes (name and notes read-only). Slot picker uses `SlotBindingHelperService.validOptions()`. Local buffer for `pendingSlotLabel` — Cancel/X discard without server call. OK routes through `canvasBlocksService.rebindModelSlot(...)` for a single evaluate roundtrip. Pencil icon in `model-settings-modal` uncommented and wired (`[attr.automationID]="'Button-Edit-Model-' + model.id"`). Title: `Edit Model`; action: `OK`. Invalid-current banner shown above the picker when the model's current binding is not in `validOptions()` (e.g. slot Empty / node gone). OK disabled on no-op rebind and when no valid slots exist. Awaiting smoke test
- [x] 10.3.1 Save-wire path decided: reuse the standard evaluate roundtrip via `rebindModelSlot(...)` → `updateAndPrint()`. No new IPC. Rust `reconcile_derived_state` recomputes `model_error` on the response so the healed model clears in place
- [ ] 10.4 [deferred with §4] Use the helper in the block-param editor for `SlotIndex`-typed fields
- [ ] 10.5 Virtual invalid entry inside the Edit Model slot picker is visually distinct (color / label suffix) and NOT selectable. When it is the only entry (no other valid slots), the picker still renders it (read-only) and shows a message such as "No other valid slots available; reconfigure hardware to rebind."

## 6e. Module-change taxonomy split + cross-slot event-ref flag — done

Splits blocking vs warning errors on `model_error`, clamps module-constrained event params against the current hardware catalog, and flags cross-slot notify references to missing/Empty slots without silently rewriting user data.

- [x] 6e.1 Split `ModelErrorKind` into `SystemConfig` (blocking) and `ModuleChanged` (warning) with `#[serde(rename_all = "snake_case")]`. `has_system_config_error()` narrows the blocking gate; `has_module_changed_warning()` is informational only
- [x] 6e.2 `TriggerModelState::diagnose(&SlotChannelList) -> Option<(ModelErrorKind, String)>` with precedence: snapshot missing / node missing / slot missing / Empty → `SystemConfig`; module differs → `ModuleChanged`. `diagnose_system_config` retained as compatibility wrapper
- [x] 6e.3 `Module::catalog_family() -> Option<&'static str>` (SMU/PSU) and `Module::tsp_identifier() -> Option<&'static str>` (smu/psu) consolidate the two family-lookup call sites (constraint keys + handlebars `module_type`)
- [x] 6e.4 `TriggerFlowState::clamp_module_constrained_params(&Catalog)` walks all block-parameter event refs (single `event_id`, `on event.event_id`, and `wait on event.event` list variants) and clamps any value against `constraints[family].options` when the referenced slot's module family disagrees. Fires from three sites: `process_system_config`, `handle_evaluate_request`, `handle_recall_request`. Simplified to in-place mutation on the state clone; no return value
- [x] 6e.5 Cross-slot event refs targeting a missing or Empty slot emit a per-block `block_error` note instead of silently skipping ("no longer present" / "no module installed"). Stored parameter value is preserved — Rust's flag is the signal the UI hides otherwise
- [x] 6e.6 UI mirror: `ModelErrorKind = 'system_config' | 'module_changed'` union added to `triggerFlowState.ts`. `getSectionHasModelError` and `isModelStale` narrowed to `system_config` only; warning-only kinds do not fire the red section header or disable the block panel. `modelStaleTooltip()` prefers the blocking message
- [x] 6e.7 UI `blockParameterHelper.normalizeParameterValues`: split missing-value seeding from stale-value snapping. For `slot_index`, an invalid stored value is preserved (Rust flags the block_error; snapping here would hide the signal). Other constrained params still snap to first option to keep the payload serializable
- [x] 6e.8 UI `blockParameterHelper.resolveParameterOptions` for `slot_index`: prepend the current stored value when it is not in the currently valid options. Prevents a native `<select>` from silently displaying the first valid option (e.g. slot 2) while the underlying binding is still an invalid slot (e.g. slot 1). Dropdown now truthfully reflects the stored value; user can rebind explicitly or leave the block flagged
- [x] 6e.9 Rust tests: `clamp_module_constrained_params_tests` (module-swap clamp), `flags_but_no_clamp_when_slot_module_is_empty`, `flags_but_no_clamp_when_referenced_slot_missing` (cross-slot flag path). Existing `is_stale_tests`, `model_error_tests`, `process_system_config_tests`, `recall_backfill_tests` refreshed for the taxonomy split. 47/47 passing
- [x] 6e.10 Model-modal empty-state: hides the slot dropdown and shows `"No valid slots available. Configure hardware or free a slot to create a model."` when `slotOptions.length === 0`. Create button disabled with tooltip via `disableCreate` / `createDisabledReason` getters. `main-flow.ts::slotOptions` refactored to a `computed()` signal fed by `SlotBindingHelperService.validOptions()`; an `effect()` snaps `modelSlot` / `modelNodeId` when the current selection becomes invalid
...
- [ ] 13.7 Preserve block selection across mid-session Systems updates. Root cause found via diagnostic trace: the recall response from `handle_recall_request` never reaches the UI — recall arrives via `StdinLine::SessionData` before the WebSocket reconnects, so the response is written to a not-yet-connected socket and lost. The UI only learns about recalled models via the follow-up Systems response, which currently must carry catalog so the UI takes its full-rebuild path (`loadSessionData`). The `is_recall_completion` heuristic in `process_system_config` (attach catalog when models exist) is doing that job — but it also fires on every mid-session hardware change, forcing a full rebuild that wipes the user's block selection. **Attempted this session and reverted.** Direct swap (drop `is_recall_completion`; UI gates rebuild on `payload.catalog || hasUnknownModel`) leaves a scenario broken: if the UI joins AFTER both SessionData and the follow-up Systems have arrived (recall response lost, subsequent Systems ships no catalog), the UI never receives catalog and rebuild has nothing to render against. Correct fix requires either (a) implementing an actual `InitialRequest` handler that returns `initial_response` with slot list + catalog so the UI can bootstrap on any connect, OR (b) a hedged rule like `attach_catalog = is_fresh_init || self.catalog.is_none()` treating Rust's copy as a proxy for "have we ever emitted catalog?" (imperfect — Rust's copy can be `Some` after recall while UI never received it). Defer until scope decision on `InitialRequest` handler
- [x] 13.8 Channel allocation: each channel is identified by its node, slot, and channel number. Models with a blocking `SystemConfig` error do not occupy capacity; healthy models and models with a `ModuleChanged` warning do. Rust recalculates each channel’s `in_use` flag after Systems, evaluate, and recall processing, for both local and linked-node hardware. The Models dialog displays the resulting `used/total` channel count, and Add Model is disabled when no slot can accept another model. Tests cover channel conflicts across nodes and capacity behavior for stale and warning models.

## 15. Design polish — done this session

Reduces complexity of the slot_index / block_error pipeline. Design improvements that stand independent of new features.

- [x] 15.1 `TriggerFlowState::reconcile_derived_state(&mut self, &Catalog)` in `state.rs` — one method calling `recompute_all_model_errors()` then `clamp_module_constrained_params(catalog)` in the required order. Migrated three call sites (`process_system_config` Ok arm, `handle_evaluate_request`, `handle_recall_request`). Handlers now call one method; new rebind handlers can't forget the order
- [x] 15.2 `reconcileSlotOptions(stored, validOptions)` helper in `blockParameterHelper.ts` — prepends the stored `slot_index` to the option list when non-empty and not in `validOptions`. Called from `resolveParameterOptions` (was inline) and `specific-event.ts::optionsFor` (previously returned raw slot list, missing preservation). Removed the now-dead `if (param.name !== 'slot_index')` guard from `normalizeParameterValues` and the step-1 special-case block from `specific-event.ts::rebuild`. One rule, one implementation, four callers
- [x] 15.3 Option A: `CLAMP_NOTE_PREFIX = "Hardware: "` marker for clamp-added `block_error` entries. In `clamp_module_constrained_params`, the reset step became a `retain` that drops only entries starting with the prefix — validator-origin and any other block_error survive on heal. All three clamp note formats prefixed. Tests: `clears_block_error_when_slot_heals`, `preserves_block_error_on_stale_model`, `preserves_unrelated_block_error_on_heal`. `TODO(follow-up)` comment points to Option B — typed `BlockErrorKind` enum mirroring `ModelErrorKind`, replaces string-prefix discriminator. Deferred; low urgency until more block_error origins ship

## 14. Known issues to investigate — next session

Surfaced during exploratory testing after the §6e slot_index preservation + dropdown-truthfulness changes. Deferred so implementation scope stays clear.

- [ ] 14.1 Slot-index edge cases: user reported "some issues with the slot index changes made so far" during exploratory testing — details not yet captured. Repro checklist to run at session start: (a) valid binding → make referenced slot Empty → reopen event editor → dropdown should show the invalid slot pre-selected, block_error visible, script preserves stored value; (b) same but with the slot fully removed from hardware; (c) module swap on the referenced slot (SMU ↔ PSU) → `notify_event_number` should clamp against new family; (d) cross-slot notify with `wait on event` (list variant) → all three ref shapes must hit `clamp_one_event_ref`; (e) recall of a saved session where the referenced slot no longer exists → backfill + flag path.

- [ ] 14.2 `specific-event.ts` does not default its event `slot_index` to the parent model's slot. Pre-existing (confirmed via git: file has only one commit `60ac694` before session; `event-block.ts` has `@Input() modelSlotIndex` but `specific-event.ts` never did). Class doc explicitly says "independent of the parent block's slot". Only appears wrong when hardware has multiple non-Empty slots — with a single non-Empty slot, `options[0]` coincidentally matches. Fix (3 small edits): (a) add `@Input() modelSlotIndex = 1;` to `specific-event.ts`; (b) in `rebuild()` seed step, prefer `modelSlotIndex` for `slot_index` when it's in the valid options, else fall back to `p.default ?? options[0]`; (c) pass `[modelSlotIndex]="selectedBlockSlotIndex"` from `block-parameters.html` on the `<app-specific-event>` element (already passed to `<app-event-block>` just above). Zero risk, aligns two components. Trivial when scheduled.

- [ ] 14.3 On-event single-selection radio group: switching event type (Notify → Timer → Notify) clears the displayed params of the previously-selected event. Pre-existing (`event-block.ts::selectSingleEvent` overwrites `param.value` with a fresh event; single mode stores only one at a time). `getParamValue(type, ...)` returns `''` when the type isn't in `selectedEvents`, so previously-selected rows render disabled + empty. Two paths to improve, both non-trivial: (a) component-local `Map<type, params>` cache so disabled rows show their last-configured values; (b) wire storage change to keep multiple events with an `active_type` marker. Not urgent — user can reconfigure. Flag for a UX pass, priority TBD.
