...
- [ ] 3.4 In model-slot dropdown handlers (e.g., model settings modal), update `model.slot_module` when the user picks a new slot. Confirm coverage across all rebind code paths (create modal, template instantiation, any programmatic assignment). Delivered via the Edit Model rebind modal — see §10.3.
...
- [ ] 10.3 New standalone component `edit-model-modal` (sibling of `model-modal`, deliberately not a shared component per user direction). Opens on pencil-icon click in `model-settings-modal` (icon placed in the right-hand actions group, immediately left of the trash button; markup already stubbed in HTML — uncomment + rewire). Prefilled with the selected model's name, slot binding, notes. Name and notes fields **read-only for this iteration** (editable later, out of scope). Slot picker shows `SlotBindingHelperService.validOptions()` PLUS the model's currently-selected binding (read-only entry, visually distinct) when that binding is invalid. On Save, updates `model.slot_module` snapshot from the newly picked slot (delivers §3.4). Title: **"Edit Model"**; action button: **"Save"**. Uses `Pencil.svg` from `trigger-flow-ui/src/assets/shapes/Icons/`
- [ ] 10.3.1 Save-wire path: check whether an "update model" IPC exists or reuse the current pattern (mutate local state, send full-state update). Decide in-implementation
